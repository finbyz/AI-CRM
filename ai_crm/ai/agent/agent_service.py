...
class AgentService():
    ...
    def get_memory(self):
        ...
        if self.agent_doc.memory_type == "Buffer Memory":
            ...
            base_memory = ConversationBufferWindowMemory(k=self.agent_doc.window_size or 5, return_messages=True)
            ...
            base_memory = VectorStoreRetrieverMemory(retriever=self.get_vector_retriever())
    ...
    def get_vector_retriever(self):
        ...
        try:
            if hasattr(self.agent_doc, 'knowledge_base') and self.agent_doc.knowledge_base:
                kb = frappe.get_doc("Knowledge Base", self.agent_doc.knowledge_base)
